...
function buildAnimalSchema(t: TFunction) {
  return z.object({
    ...
    // Gerado automaticamente pelo servidor (ANL-004) — nunca preenchido pelo utilizador.
...
export default function Animais() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      sold: rows.filter((a) => a.status === "vendido").length,
      dead: rows.filter((a) => a.status === "morto").length,
      slaughtered: rows.filter((a) => a.status === "abatido").length,
      transferred: rows.filter((a) => a.status === "transferido").length,
    };
  }, [statsData]);
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <motion.div variants={prefersReducedMotion ? undefined : fadeInUp}>
          <AdminCard variant="glass" icon={Heart} metric={stats.females} title={t("kpi.females")} caption={t("kpi.femalesCaption")} />
        </motion.div>
      </motion.div>
      ...
      >
        <motion.div variants={prefersReducedMotion ? undefined : fadeInUp}>
          <AdminCard variant="glass" icon={DollarSign} metric={stats.sold} title={t("kpi.sold")} caption={t("kpi.soldCaption")} />
        </motion.div>
        <motion.div variants={prefersReducedMotion ? undefined : fadeInUp}>
          <AdminCard variant="glass" icon={Skull} metric={stats.dead} title={t("kpi.dead")} caption={t("kpi.deadCaption")} />
        </motion.div>
        <motion.div variants={prefersReducedMotion ? undefined : fadeInUp}>
          <AdminCard variant="glass" icon={PackageCheck} metric={stats.slaughtered} title={t("kpi.slaughtered")} caption={t("kpi.slaughteredCaption")} />
        </motion.div>
        <motion.div variants={prefersReducedMotion ? undefined : fadeInUp}>
          <AdminCard variant="glass" icon={ArrowRightLeft} metric={stats.transferred} title={t("kpi.transferred")} caption={t("kpi.transferredCaption")} />
        </motion.div>
      </motion.div>
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      {editItem ? (
                        ...
                        <Input value={t("form.placeholders.tagAutoGenerated")} disabled />
                      )}
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                ...
                <div className="space-y-2">
                  {events.map((e) => (
                    <div key={e.id} className="rounded-lg border p-3">
                      ...
                      {e.destination && (
                        <p className="text-sm">
                          <span className="text-muted-foreground">{t("detail.events.destination")}: </span>
                          {e.destination}
                        </p>
                      )}
...
function EventDialog({
  ...
  const isTransfer = eventType === "transferencia";

  const submit = async () => {
    if (isTransfer && !destination.trim()) {
      toast({ title: t("validation.destinationRequired"), variant: "destructive" });
      return;
    }
    setSaving(true);
    try {
      await createEvent.mutateAsync({
        eventType,
        eventDate,
        destination: isTransfer ? destination.trim() : null,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <div className="space-y-3">
          ...
          {isTransfer && (
            <div>
              <Label>{t("detail.events.destination")}</Label>
              <Input
                placeholder={t("detail.events.destinationPlaceholder")}
                value={destination}
